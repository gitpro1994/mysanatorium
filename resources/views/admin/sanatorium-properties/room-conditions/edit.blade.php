...
<div class="card">
    <div class="card-body">
        <form action="{{ route('admin.room-conditions.update', ['id'=>$room->id]) }}" method="POST">
            @csrf
            <div class="row mb-3">
                <label for="name" class="col-4 col-xl-3 col-form-label">Ad</label>
                <div class="col-8 col-xl-9">
                    <input type="text" value="{{ $room->name }}" class="form-control" name="name" id="name">
                    @error('name')
            ...
            <div class="row mb-3">
                ...
                <div class="col-8 col-xl-9">
                    <textarea name="description" id="description" class="form-control" cols="4" rows="4">{{ $room->description }}</textarea>
                    @error('description')
                    <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
