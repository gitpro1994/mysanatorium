...
(function (factory) {
    ...
    $.dateRangePickerLanguages =
    {
        ...
        'ru':
        {
            ...
            'default-more': 'Пожалуйста выберите диапазон больше 7 ноцей',
    ...
    $.fn.dateRangePicker = function (opt) {
        ...
        opt = $.extend(true,
            {
                ...
                hoveringTooltip: function (days, startTime, hoveringTime) {
                    var night = days - 1;
                    return days > 1 ? days + ' ' + lang('days') + ' - ' + night + ' ' + lang('night') : '';
                },
                showTopbar: true,
                swapTime: false,
        ...
        function showSelectedInfo(forceValid, silent) {
            ...
            if (opt.start && opt.singleDate) {
                ...
                var night_count = countDays(opt.end, opt.start) - 1;
                box.find('.selected-days').show().find('.selected-night-num').html(night_count);
                box.find('.apply-btn').removeClass('disabled');
                var dateRange = getDateString(new Date(opt.start)) + opt.separator + getDateString(new Date(opt.end));
                opt.setValue.call(selfDom, dateRange, getDateString(new Date(opt.start)), getDateString(new Date(opt.end)));
                if (initiated && !silent) {
        ...
        function createDom() {
            ...
            if (opt.showTopbar) {
                ...
                if (opt.customTopBar) {
                    ...
                    html += '<div class="normal-top">\
							<span style="color:#ffffff">' + lang('selected') + ' </span> <span class="start-day">...</span>';
                    if (!opt.singleDate) {
                        html += ' <span class="separator-day">' + opt.separator + '</span> <span class="end-day">...</span> <i class="selected-days">(<span class="selected-days-num">3</span> ' + lang('days') + ' - <span class="selected-night-num">3</span> ' + lang('night') + ')</i>'
                    }
                    html += '</div>';
            ...
            html += '<div class="month-wrapper">'
                ...
                '</th><th style="width:27px;">' + (opt.singleDate || !opt.stickyMonths ? '' +
            ...
            if (hasMonth2()) {
                html += '<div class="gap">' + getGapHTML() + '</div>'
                    ...
                    '<thead><tr class="caption"><th style="width:27px;">' + (!opt.stickyMonths ? '' +
                        '<span class="prev">&lt;</span>' +
                        '' : '') + '</th><th colspan="' + _colspan + '" class="month-name"></th>' +
                    '<th style="width:27px;">' +
                    '<i class="fa fa-chevron-right glyphicon glyphicon-chevron-right"></i>' +
                    '</th></tr><tr class="week-name">' + getWeekHead() + '' +
                    '</thead>' +
                    '<tbody>' +
                    '</tbody>' +
                    '</table>'
        ...
        function getGapHTML() {
            var html = [
                '<div class="gap-top-mask">' +
                '</div><div class="gap-bottom-mask">' +
                '</div><div class="gap-lines">'];
            for (var i = 0; i < 20; i++) {
